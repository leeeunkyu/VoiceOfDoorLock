...
function memberSafe(memberId) {
	$.ajax({
        ...
        success : function (test) {
        	console.log(test);
        	if(test) {
        		document.getElementById('blockmember').innerHTML = memberId+'가 정상적으로 차단 해제 되었습니다.';  
            	goPage(1);
        	}else{
        		document.getElementById('blockmember').innerHTML = memberId+'가 차단 해제 안됨';        		
        	}
